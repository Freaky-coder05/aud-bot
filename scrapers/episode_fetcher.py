...
async def fetch_gdshare_url(anime_url: str, target_episode: int = None) -> tuple[str | None, int]:
    ...
    async with async_playwright() as p:
        ...
        log.info(f"[480p] Opening: {anime_url}")
        await page.goto(anime_url, wait_until="domcontentloaded", timeout=60000)
        # await page.screenshot(path=_ss("480_01_main_page")) # Optional
        log.info(f"  title: {await page.title()}")
        ...
        # Using a raw string (r''') to safely pass regex escaping to JavaScript
        link_data = await page.evaluate(r'''([targetEp]) => {
            ...
            // 1. Search for exact episode WITH Multi Audio
            for (let a of p480Links) {
                let parent = a.parentElement;
                let foundEp = false;
                let foundMulti = false;

                // Traverse up the DOM tree (up to 6 levels) to check the block's text
                for (let i = 0; i < 6 && parent; i++) {
                    const text = parent.textContent;
                    // \b ensures we match "Episode 25" and not "Episode 250"
                    const epRegex = new RegExp("Episode\\s*" + targetEp + "\\b", "i");
            ...
            for (let a of p480Links) {
                ...
                for (let i = 0; i < 6 && parent; i++) {
                    const epRegex = new RegExp("Episode\\s*" + targetEp + "\\b", "i");
                    if (epRegex.test(parent.textContent)) {
        ...
        if not download1_url:
            # await page.screenshot(path=_ss("ERR_no_480p")) # Optional
        ...
        await page.goto(download1_url, wait_until="domcontentloaded", timeout=60000)
        await asyncio.sleep(3)
        # await page.screenshot(path=_ss("480_02_download1_page")) # Optional
        log.info(f"  title: {await page.title()}")
        ...
        if count == 0:
            # await page.screenshot(path=_ss("ERR_no_gdshare")) # Optional
            log.error("  ❌ Gdshare not found")
            await browser.close()
            return None, episode_num
        ...
        if href.startswith("http") and "gdshare" in href:
            ...
            try:
                ...
                await asyncio.sleep(2)
                gdshare_url = gp.url
                # await gp.screenshot(path=_ss("480_03_gdshare_tab")) # Optional
                await gp.close()
                log.info(f"  Captured from tab: {gdshare_url}")
            except Exception as e:
                log.warning(f"  Tab capture failed: {e}")
